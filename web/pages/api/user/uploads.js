    ...
    if (req.method === "GET") {
        if (session) {
            try {
                ...
                if (userUploads !== null) {
                    let results = await Promise.all(userUploads['uploads'].map(async (upload)=>{upload.thumbnail = await generatePreSignedGetUrl(thumbnailBucket, upload.uuid); return upload;}))
                    console.log(results)
